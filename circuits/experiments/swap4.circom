...
template ZKAMMSwap() {
    ...
    // Calculate the amount of y to be received
    amount_y <== y - (k \ new_x);  // This is integer division, but it's okay here as it's not in a constraint

    // Ensure the constant product formula holds
    k === new_x * (y - amount_y);

    // Assign the amount received
    amount_received <== amount_y;
